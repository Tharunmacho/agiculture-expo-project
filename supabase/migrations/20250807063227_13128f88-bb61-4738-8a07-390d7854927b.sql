-- Create comprehensive social media and chat features (final corrected version)

-- Add missing columns to existing chat_rooms table
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'user_activity_feed' AND rowsecurity = true) THEN
    ALTER TABLE public.user_activity_feed ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'notification_queue' AND rowsecurity = true) THEN
    ALTER TABLE public.notification_queue ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'user_achievements' AND rowsecurity = true) THEN
    ALTER TABLE public.user_achievements ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'community_challenges' AND rowsecurity = true) THEN
    ALTER TABLE public.community_challenges ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'challenge_participants' AND rowsecurity = true) THEN
    ALTER TABLE public.challenge_participants ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'community_leaderboard' AND rowsecurity = true) THEN
    ALTER TABLE public.community_leaderboard ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'live_streams' AND rowsecurity = true) THEN
    ALTER TABLE public.live_streams ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'stream_viewers' AND rowsecurity = true) THEN
    ALTER TABLE public.stream_viewers ENABLE ROW LEVEL SECURITY;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'comment_attachments' AND rowsecurity = true) THEN
    ALTER TABLE public.comment_attachments ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

-- RLS Policies for chat_rooms (update existing policy)
...
DROP POLICY IF EXISTS "Users can view activity feed" ON public.user_activity_feed;
DROP POLICY IF EXISTS "System can create activity feed" ON public.user_activity_feed;
...
DROP POLICY IF EXISTS "System can create notifications" ON public.notification_queue;
...
DROP POLICY IF EXISTS "Users can create challenges" ON public.community_challenges;
DROP POLICY IF EXISTS "Challenge creators can update challenges" ON public.community_challenges;
...
DROP POLICY IF EXISTS "Users can view challenge participation" ON public.challenge_participants;
DROP POLICY IF EXISTS "Users can participate in challenges" ON public.challenge_participants;
...
DROP POLICY IF EXISTS "Users can view leaderboard" ON public.community_leaderboard;
...
DROP POLICY IF EXISTS "Stream hosts can update streams" ON public.live_streams;
...
DROP POLICY IF EXISTS "Users can view stream viewers" ON public.stream_viewers;
DROP POLICY IF EXISTS "Users can join streams" ON public.stream_viewers;
DROP POLICY IF EXISTS "Users can update their stream status" ON public.stream_viewers;
...
DROP POLICY IF EXISTS "Users can view comment attachments" ON public.comment_attachments;
DROP POLICY IF EXISTS "Comment authors can manage attachments" ON public.comment_attachments;
